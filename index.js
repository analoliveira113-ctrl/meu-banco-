...
app.post('/usuarios', (req, res)=>{
    ...
    //comando SQL para inserir um novo registro na tabela 'usuaios'
    //od '?' são placeholders que serão substituidos pelos valores digitados
    const sql = `INSERT INTO usuarios (nome, email) VALUES (?, ?)`;

    db.run(sql, [nome, email], function(err){
        //se ocorrer erro pode ser email já existe retorna erro 500
        if(err){
            console.error(err.message);
            return res.status(500).json({error: 'Erro ao inserir usuario ou email já existente'});
        }
        res.status(201).json({
...
app.delete('/usuarios/:id', (req, res) =>{
    ...
    db.run(sql, [id], function(err){
        if(err){
             console.error(err.message);
        return res.status(500).json({error: 'Erro ao deleta usuário'});
        }
        if(this.changes > 0) {
            res.json({message: `Usuário com ${id} deletado com sucesso`});
        }else{
            res.status(404).json({error: 'Usuário não encontrado'});
        }
    });
...
process.on('SIGINT', () =>{
    db.close((err) =>{
        ...
        console.lof('Conexão com o banco de dados SQLite fechada');
        process.exit(0);
    });
});